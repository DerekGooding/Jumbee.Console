...
namespace ColorCode.Parsing
{
    public class LanguageParser : ILanguageParser
    {
        ...
        public void Parse(string sourceCode,
                          ILanguage language,
                          Action<ReadOnlyMemory<char>, IList<Scope>> parseHandler)
        {
                           ...
                           Action<ReadOnlyMemory<char>, IList<Scope>> parseHandler)
        {
            ...
            // Fragments are handed to the parseHandler as zero-copy ReadOnlyMemory<char> slices of the source
            // string (no per-fragment Substring allocation).
            if (!regexMatch.Success)
                parseHandler(sourceCode.AsMemory(), Array.Empty<Scope>());
            else
            {
                int currentIndex = 0;
                ...
                {
                    int beforeMatchLength = regexMatch.Index - currentIndex;
                    if (beforeMatchLength > 0)
                        parseHandler(sourceCode.AsMemory(currentIndex, beforeMatchLength), Array.Empty<Scope>());
                    ...
                    if (regexMatch.Length > 0)
                    {
                        ...
                        parseHandler(sourceCode.AsMemory(regexMatch.Index, regexMatch.Length), capturedStyleTree);
                    }
                ...
                if (currentIndex < sourceCode.Length)
                    parseHandler(sourceCode.AsMemory(currentIndex), Array.Empty<Scope>());
            }
        }
