...
public sealed class LiveDashboardExample : CompositeControl, IExample, IActivatable
{
    ...
    private void Advance()
    {
        ...
        _txUsa.Push(t, _txUsaV, maxPoints: Window);
        _txEu.Push(t, _txEuV, maxPoints: Window);
        _errors.Push(t, _errV, maxPoints: Window);
        _latency.Push(t, _latV, maxPoints: 40);
    ...
    // Populate the panels with an initial history so the dashboard looks live the instant it opens.
    private void Seed()
    {
        for (int i = 0; i < Window; i++)
        {
            ...
            _txEuV = Walk(_txEuV, 10, 45, 8);
            _errV = Walk(_errV, 2, 50, 10);
            _txUsa.Push(i, _txUsaV, maxPoints: Window);
            _txEu.Push(i, _txEuV, maxPoints: Window);
            _errors.Push(i, _errV, maxPoints: Window);
            if (i < 40) _latency.Push(i, _latV = Walk(_latV, 1, 12, 3), maxPoints: 40);
        }
