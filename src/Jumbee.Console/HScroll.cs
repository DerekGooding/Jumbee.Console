...
public struct HScroll
{
    ...
    public bool Pan(int delta, int contentWidth, int viewportWidth) => SetOffset(Offset + delta, contentWidth, viewportWidth);
    ...
    {
        Offset = Math.Clamp(Offset, 0, Max(contentWidth, viewportWidth));
        return Offset;
    }
