...
/// <summary>
/// A narrow, non-interactive column of right-aligned line numbers, highlighting the active line. Intended as an
...
public class LineNumberGutter : RenderableControl
{
    ...
    /// <summary>The zero-based active line, drawn in <see cref="ActiveStyle"/>.</summary>
    ...
    protected override IEnumerable<Segment> Render(RenderOptions options, int maxWidth)
    {
        ...
        for (var r = 0; r < rows; r++)
        {
            if (r < _lineCount)
            {
                var num = (r + 1).ToString().PadLeft(digits) + " ";
                var style = r == _activeLine ? _activeStyle : _numberStyle;
                yield return new Segment(num, style.SpectreConsoleStyle);
            }
            else
            {
