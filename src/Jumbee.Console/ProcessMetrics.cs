...
public sealed class ProcessMetrics : IDisposable
{
    ...
    public ProcessMetrics(int capacity = 128, int windowMs = 1000)
    {
        ...
        CpuSupported = !OperatingSystem.IsBrowser() && !OperatingSystem.IsWasi()
    ...
    public double BusyPercentPeak
    {
        get
        {
            ...
            {
                var period = _framePeriodMs[i];
                if (period <= 0) continue;
                var util = _frameRenderMs[i] / period * 100.0;
    ...
    {
        get { long p = 0; for (var i = 0; i < _fCount; i++) if (_frameAlloc[i] > p) p = _frameAlloc[i]; return p; }
    }
    ...
    {
        get { if (_fCount == 0) return 0; var n = 0; for (var i = 0; i < _fCount; i++) if (_frameRedrawn[i]) n++; return n * 100.0 / _fCount; }
    }
    ...
    public double DirtyAreaPercentAvg
    {
        ...
        {
            double total = 0; var n = 0;
            for (var i = 0; i < _fCount; i++) if (_frameRedrawn[i]) { total += _frameDirty[i]; n++; }
    ...
    {
        get { double p = 0; for (var i = 0; i < _fCount; i++) if (_frameDirty[i] > p) p = _frameDirty[i]; return p * 100.0; }
    }
    ...
    public double CpuUsagePercent => !CpuSupported || !TryWindow(out var o, out var n, out var ms)
                ? 0
                : Math.Max(0, (n.CpuMs - o.CpuMs) / ms * 100.0 / Math.Max(1, Environment.ProcessorCount));
    ...
    {
        get { if (_count == 0) return WorkingSetBytes; long t = 0; for (var i = 0; i < _count; i++) t += At(i).WorkingSet; return (double)t / _count; }
    }
    ...
    {
        get { long p = 0; for (var i = 0; i < _count; i++) { var w = At(i).WorkingSet; if (w > p) p = w; } return p; }
    }
    ...
    {
        var idx = (_fStart + _fCount) % _frameRenderMs.Length;
    ...
    public void Sample()
    {
        ...
        var allocated = GC.GetTotalAllocatedBytes();                        // approximate, cheap, no allocation
        var cpuMs = CpuSupported ? Environment.CpuUsage.TotalTime.TotalMilliseconds : 0;
        var gcPauseMs = GC.GetTotalPauseDuration().TotalMilliseconds;
        var exceptions = Interlocked.Read(ref _exceptions);
        var workingSet = Environment.WorkingSet;                            // instantaneous gauge; cheap, no allocation
    ...
    { if (count == 0) return 0; double t = 0; for (var i = 0; i < count; i++) t += ring[i]; return t / count; }

    private double Avg(double[] ring) => Avg(ring, _fCount);

    private double Avg(long[] ring)
    { if (_fCount == 0) return 0; long t = 0; for (var i = 0; i < _fCount; i++) t += ring[i]; return (double)t / _fCount; }

    private double Max(double[] ring)
    { double p = 0; for (var i = 0; i < _fCount; i++) if (ring[i] > p) p = ring[i]; return p; }
    ...
    private bool TryWindow(out Snapshot oldest, out Snapshot newest, out double elapsedMs)
    {
        ...
        newest = At(_count - 1);
        oldest = newest;
        var windowTicks = _windowMs * (Stopwatch.Frequency / 1000.0);
    ...
    {
        var idx = (_start + _count) % _samples.Length;
        _samples[idx] = s;
